		      ...
		      GError **error)
{
	...
	info = (PackageInfo *) g_hash_table_lookup (priv->package_info, gs_app_get_source_default (app));
	if (info == NULL)
		return TRUE;
	...
	if ((flags & GS_PLUGIN_REFINE_FLAGS_REQUIRE_SIZE) != 0 && gs_app_get_size_installed (app) == 0) {
